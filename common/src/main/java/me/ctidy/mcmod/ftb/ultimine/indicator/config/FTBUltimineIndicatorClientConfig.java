...
public interface FTBUltimineIndicatorClientConfig {
    ...
    SNBTConfig GROUP_INDICATOR = FTBUltimineClientConfig.CONFIG.addGroup("indicator")
            .withDisplayOrder(1)
            .comment("Settings of FTB Ultimine Indicator located at the right of cross-hair by default");
    BooleanValue SHOW_INDICATOR = GROUP_INDICATOR.addBoolean("enable", true)
            .comment("Whether to display the indicator");

    // ///////////////////////// SHAPE ICON /////////////////////////
    SNBTConfig GROUP_SHAPE_ICON = GROUP_INDICATOR.addGroup("shape_icon")
            .withDisplayOrder(1)
            .comment("Settings of the shape icon located at the top-center of the indicator by default");
    BooleanValue SHOW_SHAPE_ICON = GROUP_SHAPE_ICON.addBoolean("enable", true)
            .comment("Whether to display the shape icon");
    EnumValue<Positioning> SHAPE_ICON_POSITION = GROUP_SHAPE_ICON.addEnum("pos", Positioning.NAME_MAP, Positioning.CENTER)
            .withDisplayOrder(1)
            .comment("Base position of the shape icon");
    IntValue SHAPE_ICON_INSET_X = GROUP_SHAPE_ICON.addInt("inset_x", 30)
            .withDisplayOrder(2)
            .comment("X offset of the shape icon");
    IntValue SHAPE_ICON_INSET_Y = GROUP_SHAPE_ICON.addInt("inset_y", -3)
            .withDisplayOrder(3)
            .comment("Y offset of the shape icon");
    // DoubleValue SHAPE_ICON_POS_X = GROUP_SHAPE_ICON.addDouble("pos_x", 0.57, 0, 1)
    //         .withDisplayOrder(2)
    //         .comment("X position of the shape icon, in ratio");
    // DoubleValue SHAPE_ICON_POS_Y = GROUP_SHAPE_ICON.addDouble("pos_y", 0.49, 0, 1)
    //         .withDisplayOrder(3)
    //         .comment("Y position of the shape icon, in ratio");
    DoubleValue SHAPE_ICON_ANCHOR_X = GROUP_SHAPE_ICON.addDouble("anchor_x", 1, 0, 2)
            .withDisplayOrder(4)
            .comment("X anchor of the shape icon, in ratio, 0.0 - left, 1.0 - center, 2.0 - right");
    DoubleValue SHAPE_ICON_ANCHOR_Y = GROUP_SHAPE_ICON.addDouble("anchor_y", 1, 0, 2)
            .withDisplayOrder(5)
            .comment("Y anchor of the shape icon, in ratio, 0.0 - left, 1.0 - center, 2.0 - right");

    // ///////////////////////// SHAPE NAME /////////////////////////
    SNBTConfig GROUP_SHAPE_NAME = GROUP_INDICATOR.addGroup("shape_name")
            .withDisplayOrder(2)
    ...
    BooleanValue showShapeName = GROUP_SHAPE_NAME.addBoolean("enable", true)
            .comment("Whether to display the shape name");
    EnumValue<Positioning> SHAPE_NAME_POSITION = GROUP_SHAPE_NAME.addEnum("pos", Positioning.NAME_MAP, Positioning.CENTER)
            .withDisplayOrder(1)
            .comment("Base position of the shape name");
    IntValue SHAPE_NAME_INSET_X = GROUP_SHAPE_NAME.addInt("inset_x", 45)
            .withDisplayOrder(2)
            .comment("X offset of the shape name");
    IntValue SHAPE_NAME_INSET_Y = GROUP_SHAPE_NAME.addInt("inset_y", -1)
            .withDisplayOrder(3)
            .comment("Y offset of the shape name");
    // DoubleValue SHAPE_NAME_POS_X = GROUP_SHAPE_NAME.addDouble("pos_x", 0.6, 0, 1)
    //         .withDisplayOrder(2)
    //         .comment("X position of the shape name, in ratio");
    // DoubleValue SHAPE_NAME_POS_Y = GROUP_SHAPE_NAME.addDouble("pos_y", 0.49, 0, 1)
    //         .withDisplayOrder(3)
    //         .comment("Y position of the shape name, in ratio");
    DoubleValue SHAPE_NAME_ANCHOR_X = GROUP_SHAPE_NAME.addDouble("anchor_x", 0, 0, 2)
            .withDisplayOrder(4)
            .comment("X anchor of the shape name, in ratio, 0.0 - left, 1.0 - center, 2.0 - right");
    DoubleValue SHAPE_NAME_ANCHOR_Y = GROUP_SHAPE_NAME.addDouble("anchor_y", 1, 0, 2)
            .withDisplayOrder(5)
            .comment("Y anchor of the shape name, in ratio, 0.0 - left, 1.0 - center, 2.0 - right");

    // ///////////////////////// ULTIMINE STATUS /////////////////////////
    SNBTConfig GROUP_ULTIMINE_STATUS = GROUP_INDICATOR.addGroup("ultimine_status")
            .withDisplayOrder(3)
            .comment("Settings of the ultimine status below the shape icon by default (Mining 64 blocks, on cooldown, etc)");
    BooleanValue SHOW_ULTIMINE_STATUS = GROUP_ULTIMINE_STATUS.addBoolean("enable", true)
            .comment("Whether to display the ultimine status");
    EnumValue<Positioning> ULTIMINE_STATUS_POSITION = GROUP_ULTIMINE_STATUS.addEnum("pos", Positioning.NAME_MAP, Positioning.CENTER)
            .withDisplayOrder(1)
            .comment("Base position of the ultimine status");
    IntValue ULTIMINE_STATUS_INSET_X = GROUP_ULTIMINE_STATUS.addInt("inset_x", 30)
            .withDisplayOrder(2)
            .comment("X offset of the ultimine status");
    IntValue ULTIMINE_STATUS_INSET_Y = GROUP_ULTIMINE_STATUS.addInt("inset_y", 9)
            .withDisplayOrder(3)
            .comment("Y offset of the ultimine status");
    // DoubleValue ULTIMINE_STATUS_POS_X = GROUP_ULTIMINE_STATUS.addDouble("pos_x", 0.57, 0, 1)
    //         .withDisplayOrder(2)
    //         .comment("X position of the ultimine status, in ratio");
    // DoubleValue ULTIMINE_STATUS_POS_Y = GROUP_ULTIMINE_STATUS.addDouble("pos_y", 0.54, 0, 1)
    //         .withDisplayOrder(3)
    //         .comment("Y position of the ultimine status, in ratio");
    DoubleValue ULTIMINE_STATUS_ANCHOR_X = GROUP_ULTIMINE_STATUS.addDouble("anchor_x", 1, 0, 2)
            .withDisplayOrder(4)
            .comment("X anchor of the ultimine status, in ratio, 0.0 - left, 1.0 - center, 2.0 - right");
    DoubleValue ULTIMINE_STATUS_ANCHOR_Y = GROUP_ULTIMINE_STATUS.addDouble("anchor_y", 0, 0, 2)
            .withDisplayOrder(5)
